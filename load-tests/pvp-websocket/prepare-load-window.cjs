...
(async () => {
  ...
  const artifactsDir = path.resolve(process.cwd(), "load-tests", "pvp-websocket", "artifacts", "latest");
  ...
  const tokenFile = path.join(artifactsDir, "pvp_ws_tokens_load_window.json");
  ...
  const snapshotFile = path.join(artifactsDir, "pg_stat_statements_before.json");
  fs.writeFileSync(snapshotFile, JSON.stringify(snapshot, null, 2));

  console.log(JSON.stringify({
    tokenCount: tokens.length,
    tokenFile,
    pgSnapshot: snapshotFile,
