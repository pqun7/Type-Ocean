 ...
 */
export const useChallengeHandler = (userId?: string, currentChallenge?: DailyChallenge | null) => {
  // State management for challenge updates
  const [isUpdating, setIsUpdating] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [optimisticChallenge, setOptimisticChallenge] = useState(currentChallenge);
